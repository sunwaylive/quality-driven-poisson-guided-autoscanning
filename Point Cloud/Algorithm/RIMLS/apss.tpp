...
typename APSS<_MeshType>::Scalar APSS<_MeshType>::potential(const VectorType& x, int* errorMask) const
{
	...
	if (mStatus==ASS_SPHERE)
	{
		...
		return vcg::MyDot(lx,uLinear) + uConstant;
	else
	{
		return uConstant + vcg::MyDot(lx,uLinear) + uQuad * vcg::SquaredNorm(lx);
	}
}
...
typename APSS<_MeshType>::VectorType APSS<_MeshType>::project(const VectorType& x, VectorType* pNormal, int* errorMask) const
{
	...
	do {
		...
		{
			normal = uLinear;
			position = lx - uLinear * (vcg::MyDot(lx,uLinear) + uConstant);
		}
		else
		{
			// Newton iterations
			...
			dir *= ilg;
			LScalar ad = uConstant + vcg::MyDot(uLinear,lx) + uQuad * vcg::SquaredNorm(lx);
			...
			for (int i=0 ; i<2 ; ++i)
			{
				...
				ilg = 1./vcg::Norm(grad);
				delta = -(uConstant + vcg::MyDot(uLinear,p) + uQuad * vcg::SquaredNorm(p))*std::min<Scalar>(ilg,1.);
				p += dir*delta;
			}
			position = p;
...
bool APSS<_MeshType>::fit(const VectorType& x) const
{
	...
	LScalar aux4 = mSphericalParameter * LScalar(0.5) *
								(sumDotPN - invSumW*vcg::MyDot(sumP,sumN))
								/(sumDotPP - invSumW*vcg::SquaredNorm(sumP));
	uLinear = (sumN-sumP*(Scalar(2)*aux4))*invSumW;
	uConstant = -invSumW*(vcg::MyDot(uLinear,sumP) + sumDotPP*aux4);
	uQuad = aux4;
	...
	const LScalar nume = sumDotPN - invSumW * vcg::MyDot(sumP, sumN);
	const LScalar deno = sumDotPP - invSumW * vcg::MyDot(sumP, sumP);
	...
	for (uint k=0 ; k<3 ; ++k)
	{
		...
		LScalar dNume = dSumDotPN - invSumW*invSumW*( sumW*(vcg::MyDot(dSumP,sumN) + vcg::MyDot(sumP,dSumN))
																								- dSumW*vcg::MyDot(sumP,sumN));
		LScalar dDeno = dSumDotPP - invSumW*invSumW*( 2.*sumW*vcg::MyDot(dSumP,sumP)
																								- dSumW*vcg::MyDot(sumP,sumP));
		...
		dVecU0 = -invSumW*( vcg::MyDot(dVecU13,sumP) + dVecU4*sumDotPP + vcg::MyDot(uLinear,dSumP) + uQuad*dSumDotPP + dSumW*uConstant);

		grad[k] = dVecU0 + vcg::MyDot(dVecU13,vcg::Point3Cast<LScalar>(x)) + dVecU4*vcg::SquaredNorm(x) + uLinear[k] + 2.*x[k]*uQuad;
	...
	for (uint k=0 ; k<3 ; ++k)
	{
		...
		for (uint j=0 ; j<3 ; ++j)
		{
			...
			for (unsigned int i=0; i<nofSamples; i++)
			{
				...
				d2SumP += p*d2w;
				d2SumN += n*d2w;
				d2SumDotPN += d2w * vcg::MyDot(n,p);
				d2SumDotPP += d2w * vcg::MyDot(p,p);
			}
			...
			LScalar d2Nume = d2SumDotPN - invSumW*invSumW*invSumW*invSumW*(
					- 2.*sumW*mCachedGradSumW[j]*( sumW*(vcg::MyDot(dSumP,sumN)+vcg::MyDot(sumP,dSumN)) - dSumW* vcg::MyDot(sumP,sumN))
					+ sumW*sumW*( mCachedGradSumW[j]*(vcg::MyDot(dSumP,sumN)+vcg::MyDot(sumP,dSumN))
											+ sumW*(vcg::MyDot(d2SumP,sumN)	+ vcg::MyDot(sumP,d2SumN)
															+ vcg::MyDot(mCachedGradSumP[j],dSumN) + vcg::MyDot(dSumP,mCachedGradSumN[j]))
											- d2SumW*vcg::MyDot(sumP,sumN)
											- dSumW*(vcg::MyDot(mCachedGradSumP[j],sumN)+vcg::MyDot(sumP,mCachedGradSumN[j])) ));

			LScalar d2Deno = d2SumDotPP - invSumW*invSumW*invSumW*invSumW*(
					- 2.*sumW*mCachedGradSumW[j] * ( 2.*sumW*vcg::MyDot(dSumP,sumP) - dSumW*vcg::MyDot(sumP,sumP))
					+ sumW*sumW*( 2.*mCachedGradSumW[j]*(vcg::MyDot(dSumP,sumP))
											+ 2.*sumW*(vcg::MyDot(mCachedGradSumP[j],dSumP)+vcg::MyDot(d2SumP,sumP))
											- d2SumW*vcg::MyDot(sumP,sumP) - dSumW*(2.*vcg::MyDot(mCachedGradSumP[j],sumP))) );
			...
			d2u0 =  ( -dVecU0 * mCachedGradSumW[j]
								- ( vcg::MyDot(dVecU13,mCachedGradSumP[j]) + vcg::MyDot(d2u13,sumP)
									+ d2u4*sumDotPP + dVecU4*mCachedGradSumDotPP[j]
									+ vcg::MyDot(uLinear,d2SumP) + vcg::MyDot(mCachedGradULinear[j],dSumP)
									+ dSumDotPP*mCachedGradUQuad[j] + d2SumDotPP*uQuad
									+ d2SumW*uConstant + dSumW*mCachedGradUConstant[j]) ) * invSumW;

			hessian[j][k] =
							dVecU13[j] + 2.*dVecU4*x[j]
						+ d2u0 + vcg::MyDot(d2u13,vcg::Point3Cast<LScalar>(x)) + d2u4*vcg::MyDot(x,x)
