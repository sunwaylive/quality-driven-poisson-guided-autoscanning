...
extern void edgeflip(princeton::TriMesh *mesh);

// Flip the order of vertices in each face.  Turns the mesh inside out.
extern void faceflip(princeton::TriMesh *mesh);

// One iteration of umbrella-operator smoothing
extern void umbrella(princeton::TriMesh *mesh, float stepsize, bool tangent = false);

// Taubin lambda/mu mesh smoothing
extern void lmsmooth(princeton::TriMesh *mesh, int niters);

// Remove the indicated vertices from the princeton::TriMesh.
extern void remove_vertices(princeton::TriMesh *mesh, const std::vector<bool> &toremove);

// Remove vertices that aren't referenced by any face
extern void remove_unused_vertices(princeton::TriMesh *mesh);

// Remove faces as indicated by toremove.  Should probably be
// followed by a call to remove_unused_vertices()
extern void remove_faces(princeton::TriMesh *mesh, const std::vector<bool> &toremove);

// Remove long, skinny faces.  Should probably be followed by a
// call to remove_unused_vertices()
extern void remove_sliver_faces(princeton::TriMesh *mesh);

// Remap vertices according to the given table
extern void remap_verts(princeton::TriMesh *mesh, const std::vector<int> &remap_table);

// Reorder vertices in a mesh according to the order in which
// they are referenced by the tstrips or faces.
extern void reorder_verts(princeton::TriMesh *mesh);
...
extern void subdiv(princeton::TriMesh *mesh, int scheme = SUBDIV_LOOP);

// Smooth the mesh geometry
extern void smooth_mesh(princeton::TriMesh *themesh, float sigma);

// Bilateral smoothing
extern void bilateral_smooth_mesh(princeton::TriMesh *themesh, float sigma1, float sigma2);

// Diffuse an arbitrary per-vertex vector (or scalar) field
template <class T>
extern void diffuse_vector(princeton::TriMesh *themesh, std::vector<T> &field, float sigma);

// Diffuse the normals across the mesh
extern void diffuse_normals(princeton::TriMesh *themesh, float sigma);

// Diffuse the curvatures across the mesh
extern void diffuse_curv(princeton::TriMesh *themesh, float sigma);

// Diffuse the curvature derivatives across the mesh
extern void diffuse_dcurv(princeton::TriMesh *themesh, float sigma);
...
extern void inflate(princeton::TriMesh *mesh, float amount);

// Transform the mesh by the given matrix
extern void apply_xform(princeton::TriMesh *mesh, const xform &xf);

// Translate the mesh
extern void trans(princeton::TriMesh *mesh, const vec &transvec);

// Rotate the mesh by r radians
extern void rot(princeton::TriMesh *mesh, float r, const vec &axis);

// Scale the mesh - isotropic
extern void scale(princeton::TriMesh *mesh, float s);

// Scale the mesh - anisotropic in X, Y, Z
extern void scale(princeton::TriMesh *mesh, float sx, float sy, float sz);

// Scale the mesh - anisotropic in an arbitrary direction
extern void scale(princeton::TriMesh *mesh, float s, const vec &d);

// Clip mesh to the given bounding box 
extern void clip(princeton::TriMesh *mesh, const box &b);
...
extern point mesh_center_of_mass(princeton::TriMesh *mesh);
...
extern void find_comps(princeton::TriMesh *mesh, std::vector<int> &comps,
	std::vector<int> &compsizes, bool conn_vert = false);

// Select a particular connected component, and delete all other vertices from
// the mesh.
extern void select_comp(princeton::TriMesh *mesh, const std::vector<int> &comps,
	int whichcc);
...
extern void select_big_comps(princeton::TriMesh *mesh, const std::vector<int> &comps,
...
extern void select_small_comps(princeton::TriMesh *mesh, const std::vector<int> &comps,
...
extern void find_overlap(princeton::TriMesh *mesh1, princeton::TriMesh *mesh2,
...
extern void shared(princeton::TriMesh *mesh, float tol);
