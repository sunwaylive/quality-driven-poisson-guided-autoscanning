...
Supports the following operations:
	...
	v3 = v1 UPPERDOT v2;		// Actually operator^
...
static inline T faceforward(const Vec<D,T> &N, const Vec<D,T> &I,
			    const Vec<D,T> &Nref)
{
	return ((Nref UPPERDOT I) < T(0)) ? N : -N;
}

template <int D, class T>
static inline T reflect(const Vec<D,T> &I, const Vec<D,T> &N)
{
	return I - (T(2) * (N UPPERDOT I)) * N;
}
			...
			const T &eta)
{
	...
	T NUPPERDOTI = N UPPERDOT I;
	T k = T(1) - sqr(eta) * (T(1) - sqr(NUPPERDOTI));
	return (k < T(0)) ? T(0) : eta * I - (eta * NUPPERDOTI * sqrt(k)) * N;
}
...
{
	using namespace std;
	return atan2(len(v1 CROSS v2), v1 UPPERDOT v2);
}
