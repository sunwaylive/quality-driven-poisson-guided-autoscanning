...
void CoordinateFrame::Render(QGLWidget* glw,QPainter* p)
{
  ...
  if(drawaxis){
    ...
    glColor(Color4f::Black);
    Add_Ons::Cone(10,linewidth,linewidth*0.5,false);
    glPopMatrix();
  ...
  if(drawlabels){
    md.color = Color4b::Black;
    md.qFont.setBold(true);
    md.qFont.setPixelSize(12);
    float d=size+scalefactor*linewidth;
    //because p = NULL, so it doesn't work 
    if (p) {
